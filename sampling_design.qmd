...
# make a data frame of the whole domain for plotting
  k_raster_fynbos %>%
    ...
    as.data.frame(xy = TRUE) %>%
    na.omit()%>%
    mutate(cluster = as.factor(cluster))-> k_df_fynbos_masked  
...
k_raster_fynbos %>%
  ...
  as.data.frame(xy = TRUE) %>%
  na.omit()%>%
  mutate(cluster = as.factor(cluster))-> k_df_fynbos
...
#| message: false
#| warning: false
#| fig-cap: "Figure 15. Sampling options within parks.  Park boundaries are denoted by black lines, sampling locations are colored by cluster. Additional polygons delimited current flight plans (white) and areas of interest indicated by BIOSCAPE PIs (brown). Markers represent proposed vegetation sampling locations."
#| fig.width: 10
#| fig.height: 10
#| strip.white: TRUE
...
#get proposed sites
# proposed_sites <- st_read("data/manual_downloads/proposed_sites/BioSCape_sites_19May2023_NoCentralMountains.kml.kml",
#                           quiet=TRUE)

proposed_sites <- st_read("data/manual_downloads/proposed_sites/BioSCape_sites_12June2023_merged.kml",
                          quiet=TRUE)
    ...
    addPolygons(data = sampling_options_wgs84,
      ...
      addPolygons(data = team_requests%>%
                st_zm(drop = T, what = "ZM"),
                  ...
                  color = "brown",
                  group = "Requests")%>%
      # addCircleMarkers(data = proposed_sites,
      #            group = "Plots",stroke = FALSE,weight = 0,fillColor = "green") %>%
      addMarkers(data = proposed_sites,
                 group = "Plots") %>%
    addMouseCoordinates() %>%
    #addImageQuery(sampling_options_wgs84, type="mousemove", layerId = "park_name") %>%
  leaflet::addLegend(position = "bottomright",
...
#| message: false
#| warning: false
#| fig-cap: "Figure 16. Number of different clusters per park"
#| fig.width: 10
#| fig.height: 5
#| strip.white: TRUE
...
#| message: false
#| warning: false
#| fig-cap: "Figure 17.Proposed vegetation plot locations. Note: Plot locations are also included in Fig. 15. "
#| fig.width: 10
#| fig.height: 5
#| strip.white: TRUE
...
# why in all that is holy would the makers of ggmap not user the standard extent format, or at least include an automated conversion?  Poor design for sure.

  domain %>%
    st_transform(crs = "WGS84")%>%
    ext()%>%
    as.vector()%>%
      listr::list_rename("left"=xmin,"bottom"= ymin,"right"=xmax,"top"= ymax) -> bbox

cfr_basemap <- ggmap::get_stamenmap(bbox = bbox[c("left","bottom","right","top")] ,
                                    zoom = 9,
                                    messaging = FALSE,
...
ggmap(cfr_basemap)+
  geom_sf(data=flights %>%
    ...
    geom_sf(data = proposed_sites %>%
               st_transform(crs = "WGS84"),
          inherit.aes = FALSE,size=.8)+

  xlab(NULL)+ylab(NULL)

ggsave(filename = "figures/vegplots_map.tif",
       plot = vegplot_map,
       device = "tiff",
       dpi = 300,
       width = 10,
...
#| message: false
#| warning: false
#| fig-cap: "Figure 18. Mean Annual Temperature vs. Mean Annual Precipitation.  Colored dots represent clusters in the focal sites and grey areas represent the entire domain. Black dots represent proposed sampling locations."
#| fig.width: 10
#| fig.height: 5
#| strip.white: TRUE

#Get climate data at focal sites
  clim_vals_sites <-
    terra::extract(x = c(climate,
                         soil_depth,
                         #dist_from_water,
                         drought),
                   y = vect(st_transform(x = proposed_sites,
                                         crs = crs(dist_from_water))),
                   cells=TRUE,
                   xy=TRUE)%>%
    dplyr::select(-ID) %>%
...
ggplot(data = k_df_w_climate,
  ...
  xlab("Mean Annual Temperature")+
  ylab("Mean Annual Precipitation")+
  geom_point(data = clim_vals_sites,
             mapping = aes(x=CHELSA_bio10_01_V1.2,
                           y = CHELSA_bio10_12_V1.2),
             shape=18)

ggsave(filename = "figures/site_location_temp_v_precip.tif",
       plot = site_temp_vs_precip,
       device = "tiff",
       dpi = 300,
       width = 10,
       height = 5,
...
#| message: false
#| warning: false
#| fig-cap: "Figure 19. Precipitation Seasonality vs. Soil Depth.  Colored dots represent clusters in the focal sites and grey areas represent the entire domain. Black dots represent proposed sampling locations."
#| fig.width: 10
#| fig.height: 5
#| strip.white: TRUE
...
ggsave(filename = "figures/site_location_precip_seasonality_v_soil_depth.tif",
       plot = site_seasonality_vs_soil,
       device = "tiff",
       dpi = 300,
       width = 10,
       height = 5,
...
#| message: false
#| warning: false
#| fig-cap: "Figure 20. Precipitation vs. Drought.  Colored dots represent clusters in the focal sites and grey areas represent the entire domain. Black dots represent proposed sampling locations."
#| fig.width: 10
#| fig.height: 5
#| strip.white: TRUE
...
ggsave(filename = "figures/site_location_precip_seasonality_v_drought.tif",
       plot = site_seasonality_vs_drought,
       device = "tiff",
       dpi = 300,
       width = 10,
       height = 5,
...
#| message: false
#| warning: false
#| fig-cap: "Figure 21. Proportional abundance of clusters across the fynbos and the vegetation plots."
#| fig.width: 10
#| fig.height: 5
#| strip.white: TRUE

# Sample distributions across clusters
  ...
  plot_cluster_fynbos <-
    terra::extract(x = k_raster_fynbos,
                   y = vect(st_transform(x = proposed_sites,
                                         crs = crs(dist_from_water))),
                   cells=TRUE,
                   xy=TRUE)%>%
  ...
  mutate(rel_abund = total/sum(total)*100,
         Group = 'Fynbos')%>%
    bind_rows(plot_cluster_fynbos %>%
                mutate(cluster = as.factor(cluster))%>%
               group_by(cluster) %>%
               summarise(total = n()) %>%
               mutate(rel_abund = total/sum(total)*100,
                      Group = "Veg. plots"))%>%
  ggplot(mapping = aes(y=rel_abund,x=cluster,fill=Group))+
    geom_bar(stat = "identity",
             alpha=0.5,
             position = "identity")+
    ylab("Proportional abundance (percent)")+
    xlab("Cluster")+
    scale_fill_manual(name="Group",values=c("grey","darkgreen"),
                        labels=c("fynbos","vegetation plots"))
...
While some of the clusters are over-represented in our sampling (e.g., 11 and 12), others are under represented (e.g., 2, 15, 17). 